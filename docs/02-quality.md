...
本文不评估工作量。接口清单以 [04](04-server.md) 为准。页面以 [05](05-client.md) 为准。

当前项目已包含 Server 与 Client；以下阶段划分保留为设计背景。质量契约与实现差距以 [AGENTS.md](../AGENTS.md) 为准：L2/L3 应为每次运行分配独立的本地持久化目录，并在重置或清理前核验归属与测试标记。下文固定目录描述的是现有 runner，不代表这部分隔离要求已经完成。
