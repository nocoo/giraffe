...
- A full refresh has `10N + 9M + 8` logical steps: the site catalog, four account-wide lists, explicit Insights derivation, contributions, nine statistics steps per statistics repository, nine detail tabs per accessible repository, one AI assessment checkpoint per statistics repository, and publication. A scoped refresh has `19N + 1` steps: statistics, detail tabs, AI assessment and publication. One selected repository therefore has 20 steps, including exactly nine detail pages, independent of site size. Pagination is counted separately and never expands the logical denominator. Cooldowns skip statistics and AI steps while retaining planned detail work. Existing runs keep their frozen plans.
