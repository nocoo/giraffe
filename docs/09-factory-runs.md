...
普通页面复用 `prepareRefresh` 和既有 snapshots 格式，Insights 继续由已保存来源派生。每类页面独立成功后更新，不等待工厂全局发布；不是跨页面原子快照。写入与 run 的游标一起进入 fenced batch，取消后的在途结果不能覆盖快照。旧 `POST /api/refresh` 仅为兼容 API，前端不再调用；没有新增数据库表或迁移。
